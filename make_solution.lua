...
--
	...
	function premake.make_solution(sln)
		...
		_p('clean:')
		for _ ,prj in ipairs(sln.projects) do
			_p('\t@$cd /d "%s"', _MAKE.esc(path.getrelative(sln.location, prj.location)))
			_p('\t@$(MAKE) -f %s clean', _MAKE.esc(_MAKE.getmakefilename(prj, true)))
			_p('\t@$cd /d "%s"', _MAKE.esc(path.getrelative(prj.location, sln.location)))
		end
		_p('')
